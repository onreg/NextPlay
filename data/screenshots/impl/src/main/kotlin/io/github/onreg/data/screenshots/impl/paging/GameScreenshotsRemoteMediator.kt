...
public class GameScreenshotsRemoteMediator(
    ...
    override suspend fun load(
        ...
        return when (
            ...
            is NetworkResponse.Success -> {
                ...
                val items = response.body.results.map(dtoMapper::map)
                persist(
                    items = items,
                    loadType = loadType,
                    page = page,
    ...
    private suspend fun resolvePage(
        ...
        LoadType.APPEND -> {
            ...
            if (last == null) {
                ...
                val nextKey = remoteKeysDao.get(gameId, last.id)?.nextKey
                if (nextKey == null) {
                    PageResolution.Finished(endOfPaginationReached = true)
                } else {
    ...
    private suspend fun persist(
        ...
        if (loadType == LoadType.REFRESH) {
            screenshotsDao.clearByGameId(gameId)
            remoteKeysDao.clearByGameId(gameId)
        }

        screenshotsDao.insertScreenshots(items)
        val positionStart = (page - INITIAL_PAGE).toLong() * pageSize
        screenshotsDao.insertCrossRefs(
            items.mapIndexed { index, entity ->
                GameScreenshotCrossRef(
                    gameId = gameId,
