package io.github.onreg.data.game.list.impl.paging
...
public class GameRemoteMediator(
    ...
    private suspend fun persistResponse(
        ...
        val listItems = games.mapIndexed { index, game ->
            GameListEntity(
        ...
        transactionProvider.run {
            if (isRefresh) {
                gameDao.clearGameListByKey(DEFAULT_LIST_KEY)
                remoteKeysDao.clearByListKey(DEFAULT_LIST_KEY)
            }
            gameDao.insertGamesWithPlatforms(databaseBundle)
            gameDao.insertGameList(listItems)
        ...
        val result = when (loadType) {
            ...
            LoadType.APPEND -> {
                ...
                if (lastItem == null) {
                    ...
                    val remoteKey = remoteKeysDao.getRemoteKey(DEFAULT_LIST_KEY, lastItem.game.id)
                    if (remoteKey == null) {
                        NextPage.Error(
