...
- `build-logic/convention/` centralizes Gradle convention plugins and shared build scripts for the entire workspace.
...
- `data/*/api` modules define business logic contracts (interfaces, models, repositories) shared with consumers.
...
- All modules adhere to the standard `src/main`, `src/test`, and `src/androidTest` layout; keep previews, assets, and fixtures near their owners to preserve clear boundaries.

## Build, Test, and Development Commands

- `./gradlew :app:assembleDebug` – compile the debug APK with the convention-managed SDK + Compose
  stack.
